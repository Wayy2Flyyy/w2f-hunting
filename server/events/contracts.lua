...
RegisterNetEvent('dd-hunting:sv:acceptContract', function(boardKey, contractId)
    ...
    if not ok then
        Bridge.Framework.ShowNotification(src, ('Accept failed: %s'):format(result), 'error')
        return
    end

    Bridge.Framework.ShowNotification(src, ('Contract accepted: %s (%s)'):format(result.label, result.id), 'success')
end)
...
RegisterNetEvent('dd-hunting:sv:abandonContract', function(contractId)
    ...
    if not ok then
        Bridge.Framework.ShowNotification(src, ('Abandon failed: %s'):format(err), 'error')
        return
    end

    Bridge.Framework.ShowNotification(src, 'Contract abandoned.', 'warning')
end)
...
RegisterNetEvent('dd-hunting:sv:turnInContract', function(contractId)
    ...
    if not ok then
        Bridge.Framework.ShowNotification(src, ('Turn-in failed: %s'):format(reward), 'error')
        return
    end

    Bridge.Framework.ShowNotification(src, ('Contract complete! +$%s, +%s XP'):format(reward.payout, reward.xp), 'success')

    if reward.inspection and reward.inspection.inspectionTriggered then
        Bridge.Framework.ShowNotification(src, ('Ranger inspection triggered. Fine: $%s | Seized: %s'):format(
...
RegisterNetEvent('dd-hunting:sv:refreshContractBoard', function(boardKey)
    ...
    if not ok then
        Bridge.Framework.ShowNotification(src, ('Refresh failed: %s'):format(err), 'error')
        return
    end

    Bridge.Framework.ShowNotification(src, 'Contract board refreshed.', 'success')
end)
